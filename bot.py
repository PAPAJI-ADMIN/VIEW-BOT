...
# ==================== DATABASE (Synchronous for stability) ====================
def init_db():
...
async def main_handler(event):
    ...
    if event.is_channel:
        ...
        # Check if it's the target ID or registered in DB
        ...
        if chat_id == TARGET_CHANNEL_ID or owner_id:
            ...
            except Exception as e: logger.error(f"Auto-order error: {e}")
        return
    ...
    if event.is_private:
        ...
        if text.startswith("/addbal") and uid == ADMIN_ID:
            try:
                ...
                update_bal(tid, amt); await event.respond(f"✅ Added ₹{amt} to {tid}")
        ...
        if uid in user_states:
            ...
            if state == "wait_link":
                ...
                try:
                    ...
                    if balance < cost: user_states.pop(uid); return await event.respond("⚠️ Insufficient Balance!")
...
async def main():
    ...
    logger.info("Bot Started - Full Features Restored")
