...
# TARGET CHANNEL ID (Hardcoded for guaranteed access)
...
async def unified_handler(event):
    ...
    # 1. HANDLE CHANNEL POSTS (GUARANTEED FOR YOUR ID)
    if event.is_channel:
        ...
        # If it matches your target ID or any registered ID
        if chat_id == TARGET_CHANNEL_ID:
            # For hardcoded ID, we need to know who to charge. 
            # We'll check the database for who registered it, or default to ADMIN if not found.
            async with aiosqlite.connect(DB_NAME) as db:
                async with db.execute("SELECT user_id FROM auto_channels WHERE channel_id = ?", (chat_id,)) as cursor:
                    row = await cursor.fetchone()
                    reg_uid = row[0] if row else ADMIN_ID
            
            logger.info(f"TARGET MATCH! Charging User: {reg_uid}")
            try:
                chat = await event.get_chat()
                is_pub = bool(chat.username)
                link = f"https://t.me/{chat.username}/{event.id}" if is_pub else f"https://t.me/c/{chat_id}/{event.id}"
...
async def main():
    ...
    logger.info("Bot Started - Target ID Mode")
