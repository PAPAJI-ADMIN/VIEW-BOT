...
# Pricing
PRICE_PER_1000 = 1.0
AUTO_VIEW_QTY = 500 # Default qty for auto-views
...
async def channel_post_handler(event):
    if event.is_channel:
        ...
        for reg_id in registered_channels:
            if str(reg_id) in str(channel_id):
                is_registered = True
                break
        ...
        if is_registered:
            ...
            # Place Auto-Order
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(None, place_smm_order, post_link, AUTO_VIEW_QTY, SMM_PRIVATE_SERVICE_ID)
            
            if "order" in result:
                logger.info(f"Auto-order placed for {post_link}")
