import os
import logging
import requests
import aiosqlite
import re
import asyncio
from telethon import TelegramClient, events, Button

# ==================== CONFIGURATION ====================
logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
logger = logging.getLogger(__name__)

API_ID = int(os.getenv("API_ID", "30208136"))
API_HASH = os.getenv("API_HASH", "8d13aabfe7e3b2c24ad507edb48f27a5")
BOT_TOKEN = os.getenv("BOT_TOKEN", "8eeafab20782b8c9ac67c580c1d36c2c")
ADMIN_ID = int(os.getenv("ADMIN_ID", "8211510972"))

# SMM Panel Config
SMM_API_URL = "https://finesmmpanel.com/api/v2"
SMM_API_KEY = "9050b7173d7b59058c21edd5328fcaf2"
SMM_SERVICE_ID = "3918" # Public Views
SMM_PRIVATE_SERVICE_ID = "3919" # Private/HQ Views

# Pricing
PRICE_PER_1000 = 25.0
AUTO_VIEW_QTY = 400 # Default qty for auto-views (10 coins per post with PRICE_PER_1000 = 25)

# Persistent Storage Path
PERSISTENT_DIR = "data"
DB_NAME = os.path.join(PERSISTENT_DIR, "reseller_bot.db")
os.makedirs(PERSISTENT_DIR, exist_ok=True)

bot = TelegramClient("reseller_session", API_ID, API_HASH)

# ==================== DATABASE ====================
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, balance REAL DEFAULT 0.0)")
        await db.execute("CREATE TABLE IF NOT EXISTS auto_channels (channel_id INTEGER PRIMARY KEY, user_id INTEGER)")
        await db.commit()

async def get_user(user_id):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT balance FROM users WHERE user_id = ?", (user_id,)) as cursor:
            row = await cursor.fetchone()
            if not row:
                await db.execute("INSERT INTO users (user_id, balance) VALUES (?, 0.0)", (user_id,))
                await db.commit()
                return 0.0
            return row[0]

async def update_balance(user_id, amount):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (amount, user_id))
        await db.commit()

async def add_auto_channel(channel_id, user_id):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT OR REPLACE INTO auto_channels (channel_id, user_id) VALUES (?, ?)", (channel_id, user_id))
        await db.commit()

async def get_all_auto_channels():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT channel_id FROM auto_channels") as cursor:
            rows = await cursor.fetchall()
            return [row[0] for row in rows]

# ==================== SMM FUNCTIONS ====================
def place_smm_order(link, quantity, service_id):
    payload = {
        "key": SMM_API_KEY,
        "action": "add",
        "service": service_id,
        "link": link,
        "quantity": quantity
    }
    try:
        response = requests.post(SMM_API_URL, data=payload, timeout=10)
        return response.json()
    except Exception as e:
        logger.error(f"SMM Panel Error: {e}")
        return {"error": str(e)}

# ==================== BOT HANDLERS ====================
user_states = {}

@bot.on(events.NewMessage(pattern='/start'))
async def start(event):
    uid = event.sender_id
    balance = await get_user(uid)
    
    welcome_text = (
        f"🚀 **Welcome to Premium View Booster!**\n\n"
        f"💰 **Your Balance:** `₹{balance:.2f}`\n"
        f"📈 **Price:** `₹{PRICE_PER_1000} per 1000 views`"
    )
    
    buttons = [
        [Button.inline("🚀 Order Views (Manual)", data="order")],
        [Button.inline("🤖 Auto-View Setup", data="auto_setup")],
        [Button.inline("💳 Add Funds", data="add_funds"), Button.inline("📊 My Account", data="account")]
    ]
    
    if uid == ADMIN_ID:
        buttons.append([Button.inline("🛠 Admin Panel", data="admin")])
        
    await event.respond(welcome_text, buttons=buttons)

@bot.on(events.CallbackQuery())
async def callback(event):
    data = event.data.decode()
    uid = event.sender_id
    
    if data == "order":
        user_states[uid] = "wait_link"
        await event.respond("🔗 **Send the Channel Post Link:**\nExample: `https://t.me/c/123456/789` (private) or `https://t.me/channel/789` (public)")
    
    elif data == "auto_setup":
        await event.respond(
            "🤖 **Auto-View Setup Instructions:**\n\n"
            "1. मुझे अपने चैनल में **Admin** बनायें।\n"
            "2. चैनल से कोई भी एक पोस्ट मुझे **Forward** करें।\n"
            "3. उसके बाद हर नई पोस्ट पर ऑटोमैटिक व्यूज आयेंगे!"
        )

    elif data == "add_funds":
        await event.respond(f"💳 **To Add Funds:**\n\nContact Admin: @YourUsername\nSend your User ID: `{uid}`")
    
    elif data == "account":
        balance = await get_user(uid)
        await event.respond(f"👤 **Account Details:**\n\n• **User ID:** `{uid}`\n• **Balance:** `₹{balance:.2f}`")

    elif data == "admin" and uid == ADMIN_ID:
        await event.respond("🛠 **Admin Panel:**\n\nUse `/addbal <user_id> <amount>` to add funds.", buttons=[Button.inline("🔙 Back", data="back")])

    elif data == "back":
        user_states.pop(uid, None)
        await start(event)

@bot.on(events.NewMessage())
async def msg_handler(event):
    uid = event.sender_id
    text = event.text.strip()

    # Admin Command: /addbal
    if text.startswith("/addbal") and uid == ADMIN_ID:
        try:
            parts = text.split()
            target_id, amount = int(parts[1]), float(parts[2])
            await update_balance(target_id, amount)
            await event.respond(f"✅ Added `₹{amount}` to `{target_id}`")
            await bot.send_message(target_id, f"🎉 **Funds Added!**\n`₹{amount}` has been added.")
        except:
            await event.respond("❌ Usage: `/addbal <user_id> <amount>`")
        return

    # Auto-Register Channel via Forward
    if event.fwd_from and event.fwd_from.from_id and not event.is_channel:
        try:
            channel_id = event.fwd_from.from_id.channel_id
            await add_auto_channel(channel_id, uid)
            await event.respond(f"✅ **Channel Registered for Auto-Views!**\nID: `{channel_id}`")
            return
        except: pass

    if text.startswith('/'): return

    # Manual Order Handling
    if uid in user_states:
        state = user_states[uid]
        if state == "wait_link":
            user_states[uid] = {"state": "wait_qty", "link": text}
            await event.respond("🔢 **Enter Quantity:** (Min 100)")
        elif isinstance(state, dict) and state.get("state") == "wait_qty":
            try:
                qty = int(text)
                link = state["link"]
                is_private = "/c/" in link
                service_id = SMM_PRIVATE_SERVICE_ID if is_private else SMM_SERVICE_ID
                cost = (qty / 1000) * PRICE_PER_1000
                balance = await get_user(uid)
                
                if balance < cost:
                    user_states.pop(uid)
                    return await event.respond("⚠️ Insufficient Balance!")
                
                user_states.pop(uid)
                msg = await event.respond("⏳ Processing...")
                loop = asyncio.get_event_loop()
                order = await loop.run_in_executor(None, place_smm_order, link, qty, service_id)
                
                if order and "order" in order:
                    await update_balance(uid, -cost)
                    await msg.edit(f"✅ **Order Successful!** ID: `{order['order']}`")
                else:
                    await msg.edit(f"❌ Order Failed: `{order.get('error', 'Error')}`")
            except:
                await event.respond("❌ Invalid Quantity.")

@bot.on(events.NewMessage)
async def channel_post_handler(event):
    if event.is_channel:
        channel_id = event.chat_id
        registered_channels = await get_all_auto_channels()
        
        # Check if channel is registered (handling different ID formats)
        # We need to ensure the channel_id in DB matches the event.chat_id
        # Telethon channel_id for private channels starts with -100, for public it's just the ID
        # The DB stores the raw channel_id from event.fwd_from.from_id.channel_id which is positive
        # So we need to compare absolute values or handle the -100 prefix
        is_registered = False
        for reg_id in registered_channels:
            # Compare absolute values for robustness
            if abs(reg_id) == abs(channel_id):
                is_registered = True
                break
        
        if is_registered:
            # Determine if channel is public or private and construct link accordingly
            channel_entity = await event.get_chat()
            is_public_channel = bool(channel_entity.username)

            if is_public_channel:
                post_link = f"https://t.me/{channel_entity.username}/{event.id}"
                service_to_use = SMM_SERVICE_ID
            else:
                clean_id = str(abs(channel_id)).replace("100", "", 1)
                post_link = f"https://t.me/c/{clean_id}/{event.id}"
                service_to_use = SMM_PRIVATE_SERVICE_ID

            # Get the user who registered this channel
            async with aiosqlite.connect(DB_NAME) as db:
                async with db.execute("SELECT user_id FROM auto_channels WHERE channel_id = ?", (channel_id,)) as cursor:
                    reg_user_row = await cursor.fetchone()
                    if not reg_user_row:
                        logger.error(f"Registered user not found for channel {channel_id}")
                        return
                    registered_user_id = reg_user_row[0]

            cost = (AUTO_VIEW_QTY / 1000) * PRICE_PER_1000
            user_balance = await get_user(registered_user_id)

            if user_balance < cost:
                logger.warning(f"Insufficient balance for user {registered_user_id} to place auto-order for {post_link}")
                await bot.send_message(registered_user_id, f"⚠️ **Insufficient Balance for Auto-Views!**\n\nYour balance (`₹{user_balance:.2f}`) is too low to place an auto-order for {post_link}. Please add funds.")
                return

            # Place Auto-Order
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(None, place_smm_order, post_link, AUTO_VIEW_QTY, service_to_use)
            
            if "order" in result:
                await update_balance(registered_user_id, -cost)
                logger.info(f"Auto-order placed for {post_link} and {cost:.2f} deducted from user {registered_user_id}")
                # Notify the admin/user who registered the channel
                try:
                    # Find who registered this channel (we'll notify the ADMIN_ID for simplicity or you can lookup DB)
                    await bot.send_message(ADMIN_ID, f"✅ **New Post Detected!**\n\n🔗 Link: {post_link}\n📈 Order for **{AUTO_VIEW_QTY} views** has been placed.\nOrder ID: `{result['order']}`")
                except Exception as e:
                    logger.error(f"Failed to send notification: {e}")
            else:
                logger.error(f"Auto-order failed: {result.get('error')}")

# ==================== RUN ====================
async def main():
    await init_db()
    await bot.start(bot_token=BOT_TOKEN)
    logger.info("Bot Started with All Features")
    await bot.run_until_disconnected()

if __name__ == "__main__":
    asyncio.run(main())
