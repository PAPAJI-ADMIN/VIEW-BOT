...
SMM_SERVICE_ID = os.getenv("SMM_SERVICE_ID", "3918") # Corrected Service ID for Telegram views

# Pricing (Price per 1000 views in your currency, e.g., ₹20)
# Updated to match the price of service 3918 (₹1.00 per 1000 views)
    ...
    if uid in user_states:
        ...
        if state == "wait_link":
            ...
            try:
                ...
                # Place SMM Order
                # We use loop.run_in_executor to avoid blocking the event loop with the sync requests call
                loop = asyncio.get_event_loop()
                order = await loop.run_in_executor(None, place_smm_order, link, qty)
