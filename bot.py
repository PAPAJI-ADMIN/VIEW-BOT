...
# [!] Restored Environment Variable logic to fix the auth error
...
# --- SERVICE SETTINGS (GUARANTEED 10s INSTANT START) ---
# ID 20130: Telegram Post Views - 100% Real - Instant Start⚡ - Fast Speed🚀
SMM_SERVICE_ID = "20130" 
SMM_PRIVATE_SERVICE_ID = "20130"

# --- BUSINESS PRICING ---
# Cost is ₹0.42 per 1k, selling at ₹5.00 per 1k (High Profit!)
PRICE_PER_1000 = 5.0
AUTO_VIEW_QTY = 100
MIN_ORDER = 1000 # Service 20130 has a 1000 min order requirement
MAX_ORDER = 2000000
...
async def handle_messages(event):
    ...
    if event.is_channel:
        ...
        try:
            ...
            # Use 1000 for auto-views if needed due to service minimum
            ...
            if res and "order" in res:
                ...
                db_exec("INSERT INTO orders (order_id, user_id, link, qty, cost, status, date) VALUES (?, ?, ?, ?, ?, ?, ?)",
                       (order_id, owner_id, post_link, order_qty, cost, "Success", datetime.datetime.now().strftime("%Y-%m-%d %H:%M")), commit=True)
                
                success_msg = (f"🚀 **INSTANT BOOST ACTIVATED!**\n\n"
                               f"🔗 **Post:** [View Post]({post_link})\n"
                               f"📈 **Quantity:** `{order_qty}` Views\n"
                               f"💰 **Cost:** `₹{cost:.2f}`\n"
                               f"🆔 **Order ID:** `{order_id}`\n\n"
    ...
    if event.is_private:
        ...
        if text == "/start":
            welcome = (f"💎 **PREMIUM VIEW BOOSTER PRO** 💎\n\n"
                       f"👤 **User ID:** `{uid}`\n"
                       f"💰 **Balance:** `₹{user[1]:.2f}`\n"
                       f"⚡ **Speed:** `10-Second Guaranteed Start`⚡")
            buttons = [
        ...
        if event.fwd_from:
            try:
                ...
                if hasattr(fid, 'channel_id'):
                    ...
                    await event.respond(f"✅ **Channel Linked!** ID: `{cid}`")
            except: pass
            return
        ...
        if uid in user_states:
            ...
            if state == "wait_link":
                ...
                try:
                    ...
                    if qty < MIN_ORDER: return await event.respond(f"❌ Minimum order is {MIN_ORDER}.")
...
async def main():
    try:
        ...
        if "api_id" not in str(e).lower(): await main()
