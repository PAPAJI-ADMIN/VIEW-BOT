...
async def main_handler(event):
    ...
    # 1. CHANNEL POST DETECTION (ENHANCED FOR PUBLIC CHANNELS)
    if event.is_channel:
        raw_cid = str(abs(event.chat_id))
        if raw_cid.startswith("100"): raw_cid = raw_cid[3:]
        
        logger.info(f"POST DETECTED IN CHANNEL: {raw_cid}")

        owner_id = get_channel_owner(raw_cid)
        if raw_cid == TARGET_CHANNEL_ID or owner_id:
            reg_uid = owner_id if owner_id else ADMIN_ID
            try:
                chat = await event.get_chat()
                # IMPORTANT: For public channels, use username. For private, use /c/ link.
                if hasattr(chat, 'username') and chat.username:
                    post_link = f"https://t.me/{chat.username}/{event.id}"
                    service_id = SMM_SERVICE_ID # Public Instant
                    logger.info(f"PUBLIC CHANNEL DETECTED: {chat.username}")
                else:
                    post_link = f"https://t.me/c/{raw_cid}/{event.id}"
                    service_id = SMM_PRIVATE_SERVICE_ID # Private HQ
                    logger.info("PRIVATE CHANNEL DETECTED")
                
                cost = (AUTO_VIEW_QTY / 1000) * PRICE_PER_1000
                balance = get_user_bal(reg_uid)
                
                if balance < cost:
                    await bot.send_message(reg_uid, f"⚠️ **Low Balance for Auto-Views!**\nPost: {post_link}")
                    return
                ...
                if res and "order" in res:
                    ...
                    logger.error(f"SMM Panel rejected order: {res}")
            except Exception as e:
                logger.error(f"Auto-order handler error: {e}")
        return
...
async def main():
    ...
    logger.info("Bot Started - Public Channel Fix Active")
