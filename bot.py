...
SMM_API_KEY = os.getenv("SMM_API_KEY", "9050b7173d7b59058c21edd5328fcaf2")
SMM_SERVICE_ID = os.getenv("SMM_SERVICE_ID", "3918") # Service ID for Public Telegram views
SMM_PRIVATE_SERVICE_ID = os.getenv("SMM_PRIVATE_SERVICE_ID", "3919") # Service ID for Private Telegram views (HQ)

# Pricing (Price per 1000 views in your currency, e.g., ₹20)
PRICE_PER_1000 = float(os.getenv("PRICE_PER_1000", "1.0"))
...
# ==================== SMM FUNCTIONS ====================
def place_smm_order(link, quantity, service_id_to_use):
    if SMM_API_KEY == "YOUR_API_KEY":
        return {"error": "SMM API Key not configured. Please contact admin."}
    
    payload = {
        "key": SMM_API_KEY,
        "action": "add",
        "service": service_id_to_use,
        "link": link,
        "quantity": quantity
    }
    try:
        # Note: requests is synchronous. In a high-traffic bot, use aiohttp.
...
async def msg_handler(event):
    ...
    # Handle Admin Commands first, regardless of state
    ...
    if uid in user_states:
        ...
        if state == "wait_link":
            ...
            try:
                qty = int(text)
                if qty < 100:
                    return await event.respond("❌ Minimum quantity is 100.")
                
                link = state["link"]
                
                # Regex for private channel link: https://t.me/c/123456/789
                is_private_channel = "/c/" in link
                
                service_id_to_use = SMM_SERVICE_ID
                if is_private_channel:
                    service_id_to_use = SMM_PRIVATE_SERVICE_ID
                ...
                order = await loop.run_in_executor(None, place_smm_order, link, qty, service_id_to_use)
                
                if order and "order" in order:
                    await update_balance(uid, -cost)
                    await msg.edit(f"✅ **Order Successful!**\n\n• **Order ID:** `{order['order']}`\n• **Type:** `{'Private' if is_private_channel else 'Public'}`\n• **Quantity:** `{qty}`\n• **Cost:** `₹{cost:.2f}`\n\nViews will start increasing shortly!")
                else:
                    error_msg = order.get("error", "Panel Error") if order else "Unknown Error"
