...
# ==================== SMM FUNCTIONS ====================
async def place_smm_order(link, quantity, service_id):
    payload = {
        "key": SMM_API_KEY,
        "action": "add",
        "service": service_id,
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(SMM_API_URL, data=payload, timeout=10) as response:
...
# ==================== BOT HANDLERS ====================
user_states = {}
channel_cache = {} # Local cache for instant lookup

async def load_channel_cache():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT channel_id, user_id FROM auto_channels") as cursor:
            rows = await cursor.fetchall()
            for row in rows:
                channel_cache[abs(row[0])] = row[1]
...
async def msg_handler(event):
    ...
    if event.fwd_from and event.fwd_from.from_id and not event.is_channel:
        try:
            ...
            channel_cache[abs(channel_id)] = uid # Update cache
    ...
    if uid in user_states:
        ...
        if state == "wait_link":
            ...
            try:
                ...
                order = await place_smm_order(link, qty, service_id)
...
async def channel_post_handler(event):
    if event.is_channel:
        abs_channel_id = abs(event.chat_id)
        
        # Instant lookup via cache
        if abs_channel_id in channel_cache:
            registered_user_id = channel_cache[abs_channel_id]
            
            # Use async tasks to run things in parallel for speed
            ...
            if is_public_channel:
                ...
                clean_id = str(abs_channel_id)
                if clean_id.startswith("100"): clean_id = clean_id[3:]
            ...
            if user_balance < cost:
                await bot.send_message(registered_user_id, f"⚠️ **Insufficient Balance!**\nBalance: `₹{user_balance:.2f}`")
                return

            # Fast Asynchronous Order
            result = await place_smm_order(post_link, AUTO_VIEW_QTY, service_to_use)
            
            if "order" in result:
                await update_balance(registered_user_id, -cost)
                # Notify the user who registered the channel
                try:
                    notification_text = (
                        f"✅ **Auto-Order Placed!**\n\n"
                        f"🔗 **Post:** {post_link}\n"
                        f"📈 **Views:** `{AUTO_VIEW_QTY}`\n"
                        f"💰 **Cost:** `₹{cost:.2f}`\n"
                        f"🆔 **Order ID:** `{result['order']}`"
                    )
                    await bot.send_message(registered_user_id, notification_text)
                except Exception as e:
...
async def main():
    await init_db()
    await load_channel_cache() # Load channels into RAM for instant speed
    await bot.start(bot_token=BOT_TOKEN)
    logger.info("Bot Started with All Features - Speed Optimized")
