...
async def msg_handler(event):
    ...
    if event.fwd_from and not event.is_channel:
        try:
            # Get the actual PeerChannel object to get the full ID
            ...
            if hasattr(fwd_id, 'channel_id'):
                ...
                logger.info(f"New Channel Registered: {cid} by {uid}")
            return
        except Exception as e:
...
async def channel_post_handler(event):
    # This handler triggers on EVERY new message
    if event.is_channel:
        # Get absolute channel ID (without -100 prefix for database matching)
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            async with db.execute("SELECT user_id FROM auto_channels WHERE channel_id = ?", (raw_cid,)) as cursor:
                ...
                if row:
                    uid = row[0]
                    logger.info(f"Match found! User: {uid}")
                    try:
...
async def main():
    ...
    logger.info("Bot Started - Robust Detection Mode")
