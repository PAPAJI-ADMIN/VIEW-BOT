...
# SMM Panel Config - LuvSMM (UPI SUPPORTED)
SMM_API_URL = "https://luvsmm.com/api/v2"
SMM_API_KEY = "de3115e273234223a01a392efef95be5" 

# FASTEST SINGLE POST SERVICE IDs (Fixed to avoid "Future Post" spam)
# ID 637: Telegram Post Views [ 1 Post ] FASTEST (0-30s Start)
# ID 638: Telegram Post Views [ Max Unlimited ] | Instant Start (Ultrafast)
SMM_SERVICE_ID = "637" 
SMM_PRIVATE_SERVICE_ID = "637" 
...
def init_db():
    ...
    # New table to track processed posts and prevent duplicate orders
...
def is_post_processed(pid):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT 1 FROM processed_posts WHERE post_id = ?", (pid,))
    row = c.fetchone()
    conn.close()
    return True if row else False

def mark_post_processed(pid):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO processed_posts (post_id) VALUES (?)", (pid,))
    conn.commit()
    conn.close()
...
async def main_handler(event):
    ...
    if event.is_channel:
        ...
        # Unique Post Identifier to prevent duplicate orders
        post_uid = f"{no_prefix_id}_{event.id}"
        
        if is_post_processed(post_uid):
            return # Already handled this post

        logger.info(f"NEW POST DETECTED: {post_uid}")
        
        # Admin Alert
        await bot.send_message(ADMIN_ID, f"📢 **New Post Detected!**\nChannel: `{no_prefix_id}`\nPost ID: `{event.id}`")
        ...
        if owner_id:
            mark_post_processed(post_uid) # Mark as processed immediately
            try:
                ...
                if res and "order" in res:
                    ...
                    await bot.send_message(owner_id, f"✅ **Instant Views Ordered!**\n\n🔗 **Post:** {post_link}\n📈 **Views:** `{AUTO_VIEW_QTY}`\n💰 **Cost:** `₹{cost:.2f}`\n🆔 **Order ID:** `{res['order']}`\n\n*Views should appear within 30 seconds.*")
                else:
    ...
    if event.is_private:
        if text == "/start":
            ...
            welcome_text = (f"🚀 **Welcome to Premium View Booster!**\n\n💰 **Your Balance:** `₹{bal:.2f}`\n📈 **Price:** `₹{PRICE_PER_1000} per 1000 views` (Instant Delivery)")
...
async def main():
    ...
    logger.info("Bot Started - Single Post Instant Views Active")
