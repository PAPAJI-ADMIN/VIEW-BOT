...
# Pricing
PRICE_PER_1000 = 25.0
AUTO_VIEW_QTY = 400 # 10 coins per post
    ...
    if event.is_channel:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            async with db.execute("SELECT user_id FROM auto_channels WHERE channel_id = ?", (norm_cid,)) as cursor:
                row = await cursor.fetchone()
                if row:
                    registered_user_id = row[0]
                    try:
                        chat = await event.get_chat()
                        is_public = bool(chat.username)
                        post_link = f"https://t.me/{chat.username}/{event.id}" if is_public else f"https://t.me/c/{norm_cid}/{event.id}"
                        service_id = SMM_SERVICE_ID if is_public else SMM_PRIVATE_SERVICE_ID
                        
                        cost = (AUTO_VIEW_QTY / 1000) * PRICE_PER_1000
                        balance = await get_user(registered_user_id)
                        
                        if balance < cost:
                            await bot.send_message(registered_user_id, "⚠️ Low Balance for Auto-Views!")
                            return
                        ...
                        if res and "order" in res:
                            ...
                            await bot.send_message(registered_user_id, f"✅ **Auto-Order Placed!**\n🔗 {post_link}\n🆔 `{res['order']}`")
                    except Exception as e:
