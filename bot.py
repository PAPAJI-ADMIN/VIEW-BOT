...
def normalize_id(cid):
    """Normalize ID to a simple string of numbers without any prefix"""
    s = str(cid)
...
async def add_auto_channel(channel_id, user_id):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    logger.info(f"Registered channel: {norm_id} for user: {user_id}")
...
async def callback(event):
    ...
    if data == "order":
        ...
        await event.respond("🔗 **Send the Channel Post Link:**\nExample: `https://t.me/c/123456/789` or `https://t.me/channel/789`")
        ...
        user_states.pop(uid, None); await start(event)

@bot.on(events.NewMessage())
async def msg_handler(event):
    uid = event.sender_id; text = event.text.strip() if event.text else ""
    ...
    if event.fwd_from and event.fwd_from.from_id and not event.is_channel:
        try:
            ...
            if hasattr(fwd, 'channel_id'):
                ...
                await event.respond(f"✅ **Channel Registered!** (ID: {normalize_id(cid)})")
            return
        except: pass
...
async def channel_post_handler(event):
    if event.is_channel:
        raw_cid = event.chat_id
        norm_cid = normalize_id(raw_cid)
        logger.info(f"--- NEW POST DETECTED ---")
        logger.info(f"Raw ID: {raw_cid} | Normalized ID: {norm_cid}")

        async with aiosqlite.connect(DB_NAME) as db:
            # Deep lookup: check both normalized and raw IDs
            async with db.execute("SELECT user_id FROM auto_channels WHERE channel_id = ? OR channel_id = ?", (norm_cid, str(abs(raw_cid)))) as cursor:
                row = await cursor.fetchone()
                if row:
                    uid = row[0]
                    logger.info(f"MATCH FOUND! Registered User: {uid}")
                    try:
                        ...
                        if balance < cost:
                            await bot.send_message(uid, f"⚠️ **Low Balance!**\nPost: {link}"); return
                        ...
                        if res and "order" in res:
                            ...
                            await bot.send_message(uid, f"✅ **Auto-Order!**\n🔗 {link}\n🆔 `{res['order']}`")
                            logger.info(f"Order Placed Successfully: {res['order']}")
                        else:
                            logger.error(f"Order Placement Failed: {res}")
                    except Exception as e:
                        logger.error(f"Handler Error: {e}")
                else:
                    logger.info(f"Channel {norm_cid} not found in database.")

async def main():
    await init_db()
    await bot.start(bot_token=BOT_TOKEN)
    logger.info("Bot Started - Robust ID Mode")
