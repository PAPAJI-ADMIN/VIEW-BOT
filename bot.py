...
# Using ID 26585 - Real Post Views (FAST) - Min 10
            ...
            try:
                ...
                if balance >= cost:
                    ...
                    if res and "order" in res:
                        ...
                        await bot.send_message(owner_id, f"🚀 **Auto-Views Sent!**\nPost: {post_link}\nQty: {AUTO_VIEW_QTY}\nCost: ₹{cost:.2f}\nOrder ID: `{res['order']}`")
                        # Admin Alert
                        await bot.send_message(ADMIN_ID, f"📢 **Admin Alert:** Order placed for {owner_id}\nLink: {post_link}\nOrder ID: `{res['order']}`")
                    else:
                        await bot.send_message(ADMIN_ID, f"⚠️ **SMM API Error:**\nUser: {owner_id}\nResponse: {res}")
                else:
                    await bot.send_message(owner_id, "⚠️ **Low Balance!** Auto-views failed for your latest post.")
            except Exception as e:
                await bot.send_message(ADMIN_ID, f"❌ **System Error:** {str(e)}")
        return
        ...
        if event.fwd_from:
            try:
                ...
                if hasattr(fid, 'channel_id'):
                    ...
                    await event.respond(f"✅ **Channel Linked!** Bot will now send {AUTO_VIEW_QTY} views to every new post in this channel.")
            except: pass
            return
...
async def on_callback(event):
    ...
    if data == "manual_order":
        ...
        await event.respond(f"🤖 **Auto-View:** Add me as Admin to your channel & Forward any post here.")
    elif data == "add_funds":
        await event.respond(f"💳 **Add Funds:** Send ID `{uid}` to {ADMIN_USERNAME} for UPI Payment.")
    elif data == "stats":
        await event.respond(f"📊 **STATS**\nID: `{uid}`\nBalance: `₹{user[1]:.2f}`\nOrders: `{user[3]}`")
    elif data == "history":
        await event.respond("📜 Use /start to refresh your balance and stats.")
